...
const AGE_GROUPS = [
  ...
  { label: 'U15', tabs: [
    { label: 'Ski Men', url: 'https://ifsafreeride.org/u15-ski-men' },
    { label: 'Ski Women', url: 'https://ifsafreeride.org/u15-ski-women/' },
    { label: 'Snowboard Men', url: 'https://ifsafreeride.org/u15-snowboard-men/' },
    { label: 'Snowboard Women', url: 'https://ifsafreeride.org/u15-snowboard-women/' },
  ]},
];
...
export default function RankingsPage() {
  ...
  const handleAgeGroup = (i: number) => {
    setAgeGroup(i);
    setActive(0);
  };

  const TABS = AGE_GROUPS[ageGroup].tabs;
  ...
  return (
    <main style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 60px', fontFamily: 'system-ui', color: '#e8e8e8' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {AGE_GROUPS.map((g, i) => (
          <button key={i} onClick={() => handleAgeGroup(i)} style={{ padding: '7px 20px', borderRadius: 999, border: ageGroup === i ? '1px solid #ffcc00' : '1px solid #333', background: ageGroup === i ? '#ffcc00' : 'transparent', color: ageGroup === i ? '#000' : '#888', fontSize: 14, cursor: 'pointer', fontWeight: ageGroup === i ? 700 : 400 }}>
            {g.label}
          </button>
        ))}
      </div>
