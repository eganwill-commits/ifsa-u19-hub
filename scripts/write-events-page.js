...
export default function EventsPage() {
  ...
  const FilterBtn = ({ label, filterKey, value }: { label: string; filterKey: keyof Filters; value: string }) => {
    ...
    return (
      <button onClick={() => setFilter(filterKey, value)} style={{ padding: "5px 14px", borderRadius: 999, border: active ? "1px solid #fff" : "1px solid #2a2a2a", background: active ? "rgba(255,255,255,0.12)" : "transparent", color: active ? "#fff" : "#aaa", fontSize: 13, cursor: "pointer", fontWeight: active ? 700 : 400, transition: "all 0.1s" }}>
        {label}
      </button>
    );
  };
  ...
  return (
    <main style={{ maxWidth: 1000, margin: "0 auto", fontFamily: "system-ui", padding: "24px 16px 60px", color: "#e8e8e8" }}>
      <h1 style={{ fontSize: "clamp(22px, 5vw, 28px)", fontWeight: 800, marginBottom: 4 }}>IFSA U19 Events</h1>
      ...
      <div style={{ border: "1px solid #1e1e1e", borderRadius: 14, padding: "12px 14px", marginBottom: 20, background: "rgba(0,0,0,0.4)", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: "#aaa", width: 66, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1 }}>Discipline</span>
          <FilterBtn label="All" filterKey="discipline" value="all" />
          <FilterBtn label="Ski" filterKey="discipline" value="ski" />
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: "#aaa", width: 66, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1 }}>Stars</span>
        ...
        {filtered.map((e) => {
          ...
          return (
            <Link key={e.id} href={\`/events/\${e.id}\`} style={{ textDecoration: "none", color: "inherit" }}>
              <div style={{ border: "1px solid #1e1e1e", borderLeft: \`3px solid \${st.bar}\`, borderRadius: 12, padding: "12px 14px", display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", background: "rgba(10,10,10,0.7)", transition: "background 0.1s" }}
              ...
              >
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 999, background: st.bg, border: \`1px solid \${st.border}\`, color: st.color, fontFamily: "monospace" }}>
...
`;
fs.writeFileSync('src/app/events/page.tsx', code);
console.log('Done!');